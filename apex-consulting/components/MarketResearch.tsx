...
const COMPETITORS = [
  { name:"Your Business", ticker:"YOU",   color:"#14B8A6", pricing:"£299/mo", marketShare:"3%",  rating:4.7, growth:"+127%", strengths:["Product quality","Customer service","Agility"],         weaknesses:["Brand awareness","Marketing budget"],  radar:{ Product:88,Price:72,Brand:45,Support:90,Speed:95,Data:70 } },
...
export default function MarketResearch() {
  ...
  return (
    <section id="intelligence" className="relative py-32 overflow-hidden">
      <div className="glow-blob w-[600px] h-[400px] bg-accent-600/5 top-1/3 right-[-10%]" aria-hidden />
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity:0,y:30 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.7,ease:[0.16,1,0.3,1] }} className="max-w-2xl mb-16">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent-400 mb-4">Market Intelligence</p>
          <h2 className="text-[clamp(2rem,5vw,3.5rem)] font-black tracking-[-0.035em] text-white mb-6 leading-[1.05]">
            Know your market. <span className="accent-shimmer">Own your position.</span>
          </h2>
        ...
        <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.6 }} className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {MARKET_STATS.map(({ label,value,icon:Icon }) => (
            <div key={label} className="glass-card p-5 flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-accent-500/10 border border-accent-500/15 flex items-center justify-center shrink-0"><Icon size={18} className="text-accent-400" /></div>
        ...
        <div className="flex items-center gap-3 mb-8 flex-wrap">
          ...
          {["SaaS / B2B Tech","E-Commerce","Professional Services","HealthTech"].map((ind) => (
            <button key={ind} onClick={() => setIndustry(ind)} className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all duration-200 cursor-pointer ${ industry===ind ? "bg-accent-500/15 text-accent-400 border border-accent-500/25" : "text-stone-500 hover:text-white border border-transparent" }`}>{ind}</button>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-6">
          <div className="space-y-3">
            {COMPETITORS.map((comp,i) => (
              <motion.div key={comp.name} initial={{ opacity:0,x:-20 }} whileInView={{ opacity:1,x:0 }} viewport={{ once:true }} transition={{ duration:0.5,delay:i*0.07 }} className={`glass-card p-5 cursor-pointer transition-all duration-300 ${active===i?"border-accent-500/30 bg-accent-500/[0.03]":""}`} onClick={() => { setActive(i); setExpanded(expanded===i?null:i); }}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right hidden sm:block"><p className="text-[12px] text-stone-400">{comp.pricing}</p><div className="flex items-center gap-1 justify-end mt-0.5"><Star size={10} className="text-accent-400 fill-accent-400" /><span className="text-[11px] text-stone-500">{comp.rating}</span></div></div>
          ...
          <motion.div initial={{ opacity:0,x:40 }} whileInView={{ opacity:1,x:0 }} viewport={{ once:true }} transition={{ duration:0.7,ease:[0.16,1,0.3,1] }} className="glass-card p-6 self-start sticky top-24">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <RadarChart data={radarData} margin={{ top:4,right:20,bottom:4,left:20 }}>
                ...
                <Radar name="You"  dataKey="You"  stroke="#14B8A6" fill="#14B8A6" fillOpacity={0.15} strokeWidth={2} />
                <Radar name="Them" dataKey="Them" stroke={selected.color} fill={selected.color} fillOpacity={0.08} strokeWidth={2} />
                <Tooltip contentStyle={{ background:"#0F2825",border:"1px solid rgba(20,184,166,0.2)",borderRadius:8 }} itemStyle={{ color:"#A8A29E" }} />
              </RadarChart>
            </ResponsiveContainer>
            <div className="flex items-center gap-4 mt-2">
              <div className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-accent-400 inline-block rounded" /><span className="text-[11px] text-stone-500">You</span></div>
            ...
            <a href="#contact" className="mt-5 flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-accent-500/10 border border-accent-500/20 text-[12px] font-semibold text-accent-400 hover:bg-accent-500/20 transition-colors cursor-pointer">Request full competitor report</a>
          </motion.div>
        </div>
      </div>
    </section>
