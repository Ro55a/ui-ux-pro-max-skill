...
export default function CTASection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrors({});
    setLoading(true);
    // TODO: replace with real form submission (e.g. POST /api/contact, or a service like Resend / Formspree)
    ...
    <section id="contact" className="relative py-32 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-gold-400 mb-4">Get in Touch</p>
          <h2 className="text-[clamp(2rem,5vw,3.2rem)] font-black tracking-[-0.035em] text-white mb-5 leading-[1.05]">
            Book a free strategy call.
          </h2>
          <p className="text-stone-400 text-[1rem] leading-relaxed">
            45 minutes, no obligation. We&apos;ll give you an honest assessment of where
            you are and what we&apos;d recommend — whether we work together or not.
          </p>
        </motion.div>
            ...
            {submitted ? (
              <div className="glass-card p-12 text-center">
                ...
                <p className="text-stone-400 text-[14px] leading-relaxed">
                  We&apos;ll be in touch within one business day to confirm your call time.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} noValidate className="glass-card p-8 space-y-5">
                <div className="grid sm:grid-cols-2 gap-4">
                  {([
                    ...
                    <div key={id}>
                      <label htmlFor={id} className="block text-[11px] font-semibold uppercase tracking-widest text-stone-500 mb-2">
                        {label}
                      </label>
                      <input
                        id={id} type={type} autoComplete={id === "email" ? "email" : "name"}
                        placeholder={placeholder}
                        value={form[id]}
                        onChange={(e) => setForm((p) => ({ ...p, [id]: e.target.value }))}
                        aria-describedby={errors[id] ? `${id}-error` : undefined}
                        className={`w-full bg-white/[0.03] border rounded-xl px-4 py-3 text-[14px] text-white placeholder:text-stone-600 outline-none transition-colors ${ errors[id] ? "border-red-500/50 focus:border-red-400" : "border-white/[0.08] focus:border-gold-500/40" }`}
                      />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-white/[0.03] border rounded-xl px-4 py-3 text-[14px] text-white placeholder:text-stone-600 outline-none transition-colors ${ errors.company ? "border-red-500/50" : "border-white/[0.08] focus:border-gold-500/40" }`}
                  />
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all duration-200 cursor-pointer ${
                          selected.includes(s)
                            ...
                            : "bg-white/[0.03] text-stone-500 border border-white/[0.07] hover:text-white"
                        }`}
                      >
                        {s}
                      </button>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl px-4 py-3 text-[14px] text-white placeholder:text-stone-600 outline-none focus:border-gold-500/40 transition-colors resize-none"
                  />
                </div>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <button
                      ...
                      className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors cursor-pointer ${
                        consent ? "bg-gold-500 border-gold-500" : errors.consent ? "border-red-500" : "border-white/20 bg-white/[0.03]"
                      }`}
                    >
                      {consent && (
                        <svg width="9" height="7" viewBox="0 0 9 7" fill="none" aria-hidden>
                          <path d="M1 3.5L3.5 6L8 1" stroke="#080705" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      )}
                    </button>
                    <span className="text-[12px] text-stone-500 leading-relaxed">
                      I have read and understood the{" "}
                      <a href="/privacy" target="_blank" rel="noopener noreferrer" className="text-gold-500 hover:underline">
                        Privacy Policy
                      </a>{" "}
                      and consent to {COMPANY.name} contacting me about my enquiry. I understand I can withdraw consent at any time.
                    </span>
                  </label>
                ...
                <button
                  ...
                  className="btn-gold w-full flex items-center justify-center gap-2 text-[14px] cursor-pointer disabled:opacity-60"
                >
                  {loading
                ...
                <p className="text-[10px] text-stone-600 text-center leading-relaxed">
                  ...
                  <a href="/privacy" className="text-stone-500 hover:text-stone-400">Privacy Policy</a>.
                  We will never sell your data or send unsolicited marketing without consent.
                </p>
              </form>
            )}
          </motion.div>
          ...
          >
            {[
              ...
              <div key={label} className="glass-card p-5 flex items-center gap-4">
                <div className="w-9 h-9 rounded-lg bg-gold-500/8 border border-gold-500/12 flex items-center justify-center shrink-0">
                  <Icon size={15} className="text-gold-400" />
                </div>
                <div>
                  <p className="text-[10px] text-stone-600 uppercase tracking-wider">{label}</p>
                  {href ? (
                    <a href={href} className="text-[13px] font-medium text-white hover:text-gold-400 transition-colors mt-0.5 block">{value}</a>
                  ) : (
            ...
            <div className="glass-card p-6">
              <p className="text-[11px] font-semibold uppercase tracking-widest text-gold-400 mb-3">What happens next</p>
              <ol className="space-y-3">
                {[
                  ...
                  <li key={i} className="flex gap-3">
                    <span className="w-5 h-5 rounded-full bg-gold-500/10 border border-gold-500/15 flex items-center justify-center text-[10px] font-bold text-gold-400 shrink-0 mt-0.5">
                      {i + 1}
                    </span>
