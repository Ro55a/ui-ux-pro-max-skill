...
export default function BusinessIntelligence() {
  ...
  const runAnalysis = async () => {
    ...
    setProfile({ ...MOCK_PROFILES.default, company: query });
    setLoading(false);
  };

  return (
    <section className="relative py-32 overflow-hidden">
      <div className="glow-blob w-[600px] h-[400px] bg-accent-600/6 top-0 left-0 -translate-x-1/3" aria-hidden />
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity:0,y:30 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.7,ease:[0.16,1,0.3,1] }} className="max-w-2xl mb-16">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent-400 mb-4">Pre-Call Intelligence</p>
          ...
          <p className="text-stone-400 text-lg leading-relaxed">
            Enter a company name or domain. We surface their likely pain points, growth stage, and the exact services they need — before you dial.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.6 }} className="max-w-2xl mb-10">
          <div className="flex gap-3">
            <div className="flex-1 flex items-center gap-3 glass-card px-4 py-3 !rounded-xl border border-white/10 focus-within:border-accent-500/30 transition-colors">
            ...
            <button onClick={runAnalysis} disabled={loading} className="btn-accent flex items-center gap-2 text-[14px] whitespace-nowrap cursor-pointer disabled:opacity-60">
            ...
            <motion.div initial={{ opacity:0,y:30 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0,y:-20 }} transition={{ duration:0.6,ease:[0.16,1,0.3,1] }} className="grid lg:grid-cols-3 gap-5">
              <div className="glass-card p-6 lg:col-span-1">
                <div className="flex items-start gap-4 mb-6">
                  <div className="w-12 h-12 rounded-xl bg-accent-500/10 border border-accent-500/15 flex items-center justify-center"><Building2 size={20} className="text-accent-400" /></div>
                ...
                <div className="mt-6 p-4 rounded-xl bg-accent-500/8 border border-accent-500/15">
                  ...
                  <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <motion.div initial={{ width:0 }} animate={{ width:`${profile.score}%` }} transition={{ duration:1,delay:0.4,ease:[0.16,1,0.3,1] }} className="h-full bg-gradient-to-r from-accent-700 to-accent-400 rounded-full" />
                  </div>
              ...
              <div className="glass-card p-6">
                <div className="flex items-center gap-2 mb-5"><CheckCircle size={14} className="text-emerald-400" /><p className="text-[11px] font-semibold uppercase tracking-widest text-emerald-400">Catalyst Opportunities</p></div>
                <div className="space-y-3">
                ...
                <a href="#contact" className="mt-5 flex items-center justify-between w-full p-3 rounded-xl bg-accent-500/8 border border-accent-500/15 hover:bg-accent-500/15 transition-colors cursor-pointer">
                  <span className="text-[12px] font-semibold text-accent-400">Use this profile for outreach</span>
