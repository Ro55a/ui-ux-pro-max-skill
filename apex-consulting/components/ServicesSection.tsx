...
export default function ServicesSection() {
  return (
    <section id="services" className="relative py-32 overflow-hidden">
      <div className="glow-blob w-[600px] h-[400px] bg-accent-600/6 top-0 left-1/2 -translate-x-1/2 -translate-y-1/2" aria-hidden />

      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div
          initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} className="max-w-2xl mb-20"
        >
          <div className="section-divider" />
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent-400 mb-4">What We Do</p>
          ...
          <p className="text-stone-400 text-lg leading-relaxed">
            We embed as a senior advisory layer — no junior consultants, no cookie-cutter frameworks.
            Every engagement is built around your specific growth constraints.
          </p>
        </motion.div>
        ...
        >
          {SERVICES.map(({ icon: Icon, label, sub, desc, tags }) => (
            <motion.div key={label} variants={card} className="glass-card p-6 flex flex-col gap-5 group cursor-pointer">
              <div>
                <div className="w-10 h-10 rounded-xl bg-accent-500/10 border border-accent-500/15 flex items-center justify-center mb-4 group-hover:bg-accent-500/20 transition-colors duration-300">
                  <Icon size={18} className="text-accent-400" />
                </div>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-accent-600 mb-1">{sub}</p>
              ...
              <div className="mt-auto">
                ...
                <a href="#contact" className="flex items-center gap-1.5 text-[12px] font-semibold text-accent-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
